...
import { decideContractStep, reviseContractStep } from "@/lib/api";
import { ApprovalTimeline } from "@/components/ui/ApprovalTimeline";
import type { ApprovalStepOut } from "@/lib/types";

function ReviseModal({
  contractId, step, onClose,
}: { contractId: string; step: ApprovalStepOut; onClose: () => void }) {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      await reviseContractStep(contractId, step.id, decision, reason);
      onClose();
      router.refresh();
    } catch (e) {
...
export function ContractApprovalPanel({ contractId, steps, size = "sm" }: { contractId: string; steps: ApprovalStepOut[]; size?: "md" | "sm" }) {
  const router = useRouter();
  ...
  async function decide(decision: "approved" | "rejected") {
    ...
    setError(null);
    try {
      await decideContractStep(contractId, decision, comment || undefined);
      setRejecting(false);
      setComment("");
      router.refresh();
    } catch (e) {
  ...
  return (
    <div>
      ...
      {reviseTarget && <ReviseModal contractId={contractId} step={reviseTarget} onClose={() => setReviseTarget(null)} />}
    </div>
  );
}
