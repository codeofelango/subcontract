...
import { decidePenaltyStep, revisePenaltyStep } from "@/lib/api";
import { ApprovalTimeline } from "@/components/ui/ApprovalTimeline";
import type { ApprovalStepOut } from "@/lib/types";

function ReviseModal({
  penaltyId, step, onClose,
}: { penaltyId: string; step: ApprovalStepOut; onClose: () => void }) {
  ...
  async function submit() {
    ...
    setError(null);
    try {
      await revisePenaltyStep(penaltyId, step.id, decision, reason);
      onClose();
      router.refresh();
    } catch (e) {
...
export function PenaltyApprovalPanel({ penaltyId, steps }: { penaltyId: string; steps: ApprovalStepOut[] }) {
  const router = useRouter();
  ...
  async function decide(decision: "approved" | "rejected") {
    ...
    setError(null);
    try {
      await decidePenaltyStep(penaltyId, decision, comment || undefined);
      setRejecting(false);
      setComment("");
      router.refresh();
    } catch (e) {
  ...
  return (
    <div>
      ...
      {reviseTarget && <ReviseModal penaltyId={penaltyId} step={reviseTarget} onClose={() => setReviseTarget(null)} />}
    </div>
  );
}
