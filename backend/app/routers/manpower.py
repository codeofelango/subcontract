...
_MANPOWER_ROLES = Depends(require_roles("admin", "hr_requester"))
...
async def _load(contract_id: str, period: str, session: AsyncSession):
    ...
    inv_by_key = {(v.job_title, v.nationality): v.invoiced_amount for v in inv}
    return contract, ts, inv_by_key


@router.get("/{contract_id}", response_model=ManpowerResponse, dependencies=[_MANPOWER_ROLES])
async def get_manpower(contract_id: str, period: str, session: AsyncSession = Depends(get_session)) -> ManpowerResponse:
    contract, ts, inv_by_key = await _load(contract_id, period, session)
    ...
    for line in ts:
        invoiced = inv_by_key.get((line.job_title, line.nationality), Decimal("0"))
        ...
        total_employees += line.employee_count
        if matched:
            matched_total += contract_amount
        if abs(variance) > worst_abs:
            worst_abs = abs(variance)
            worst_row = (line.job_title, line.nationality, variance)

        sign = "+" if variance > 0 else ("−" if variance < 0 else "")
        rows.append(ManpowerRow(
            title=line.job_title, nationality=line.nationality, employeeCount=line.employee_count,
            reg=fmt_num(line.reg_hours), regRate=f"{line.reg_rate:.0f}",
    ...
    if worst_row and worst_abs >= Decimal("100"):
        title, nationality, variance = worst_row
        direction = "overbilled" if variance > 0 else "underbilled"
        note = (
            f"{title} ({nationality}) — invoiced amount {'exceeds' if variance > 0 else 'is below'} the contract-computed amount "
    ...
    return ManpowerResponse(
        ...
        rows=rows,
        total=ManpowerTotal(
            employeeCount=total_employees, contract=money(total_contract), invoiced=money(total_invoiced),
            variance=f"{'+ ' if net_variance >= 0 else '− '}{money(abs(net_variance))}",
        ),
        varianceNote=note,
...
@router.post("/{contract_id}/approve-matched", dependencies=[_MANPOWER_ROLES])
...
@router.post("/{contract_id}/dispute", dependencies=[_MANPOWER_ROLES])
